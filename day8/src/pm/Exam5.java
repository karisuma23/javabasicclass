...
객체에서는 자식클래스에서 부모클래스 쪽으로 변환되는것이 업캐스팅 
그 반대는 다운 캐스팅이다.
객체는 항상 업캐스팅은 할수 있으므로 명시저으로 적어주지 않아도 자바가 대신해준다.
하지만 객체 명시적 으로 적어 줘도 다운캐스팅 자체가 안될 떄가 있다.
잘못된 다운캐스팅을 수행하면 ClassCastException 이 발생하고 프로그램이 종료된다.

 사람    학생은 사람이다.(언제나 OK)      업캐스팅 
 학생    사람은 학생이다.(언제나는 아니다.) 다운캐스팅
 
 사람 human1 = new 사람(); 학생과 학생이 아닌 사람 모두 포함
 ...
 포함되어 있다. 그렇기 떄문에 학생으로의 다운 캐스팅은 불가능하다.
 반면에 human2는 실제 학생() 생성자로 객체를 생성했으므로 객체 내부에는 사람의 공통된 
 특성 뿐이 아니라 학생만의 속성과 기능들이 포함되어 있다. 따라서  human2 사람 자료형으로 
 저장되어 있지만 학생으로 다운캐스팅이 가능하다.
 
 class A {}      // 할아버지
 class B extends A{} // 아버지
 class C extends B{} // 손자

// 업 캐스팅  - 학생은 언제나 사람이 될수 있다. 
 B b1 = new B();
...
A a2 = new B();
B b2 = (B) a2; // 가능 , 다운 캐스팅은 경우에 따라 될수도 있다. 
C c2 = (C) a2; // 에러 발생  

무슨 타입으로 선언돼 있는지는 중요하지 않고 어떤 생성자로 생성됐는지가 중요하다. 
B() 생성자로 만들었으면 A로 캐스팅 할수 있고 C()로 만들었다면 A와B로 캐스팅 할수 있다. 
