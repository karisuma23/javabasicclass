...
이러한 예외가 발생했을 떄 이를 적절하게 처리하는 것을 '예외 처리'라고 한다. 

예외와 에러의 차이 
예외는 '연산오류, 숫자 포맷오류 등과 같이 상황에 따라 개발자가 해결할 수 있는 '오류'
여기서 해결 할수 있는 의미는 오류 자체를 수정할 수 있다는 것이 아니라
오류가 발생했을 떄 차선책으로 선택을 하는 것을 말한다.
...
이 2개 클래스 모두 Throwable 클래스의 모든 기능을 포함한다. 
...
반면 실행예외는 컴파일 전이 아니라 실행할 떄 발생하는 예외로 처리를 하지 않더라도 
실행은된다. 따라서 프로그램 실행 중에 프로그램이 강제 종료 되는 이유는 대부분 
실행 예외 때문이다. 
...
1. 일반예외 일반 예외는 예외 처리를 하지 않으면 컴파일 자체가 불가능해진다. 
   ...
   InterruptedException - Thred.sleep(시간) 매서드는 일정 시간동안 
     해당 쓰레드를 일시 정지 상태로 만드는 Thread클래스의 매서드이다.
...
연산 자체가 불가능 할떄 발생되는 실행 예외다.
분모가 0일 떄 즉 0으로 나눌떄 이다. 이렇게 계산을 하게 되면 예외가 발생된다. 
...
AraayIndexOutOfBoundException
배열의 인덱스를 잘못 사용할떄 발생 

자바에서는 예외 처리 구문이 있으면 적절히 예외가 처리됐다고 판단을 하기 떄문에
