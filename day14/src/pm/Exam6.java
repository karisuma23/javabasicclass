...
public class Exam6 {
	/*
	  ...
	  (동질 자료, 동적 할당, 추가/변경/삭제 동일한 매서드 사용)
	...
	public static void main(String[] args) {
		...
		// Quiz aList2의 전체를 i번쨰 : 값 
		
		for(int i = 1; i < linkedlist1.size()+1; i++) {
			System.out.println(i + " 번쨰 : " + linkedlist1.get(i-1));
...
값이 추가된 위치의 앞뒤 데이터 정보만 수정하면 된다. 따라서 중간에 데이터를 추가할 떄 속도
차이가 날 것이라는 것을 예상 할수 있다. 
그렇다면 장점만 있을까!!! linkedlist는 각 원소가 자신의 인덱스 정보를  갖고 있지는 않다. 
특정 인덱스 위치의 값을 가져오기 위해서는 앞에서부터 차례로 번호를 세어가면서 인덱스의 위치를 찾아야한다.
반면에 ArrayList는 데이터 자체에 인덱스 번호를 갖고 있으므로 특정 인덱스 위치의
데이터를 빠르게 찾을 수 있다.

데이터를 추가 또는 삭제할때는 LinkedList가 빠르고 데이터를 찾을떄는 ArrayList가 빠르다. 
