...
public class Exam5 {
	/*
	   ...
	   Arrays.asList() 매서드를 사용한 정적 컬렉션 객체 생성은 내부적으로는
	     ...
	     구현 클래스로 객체를 생성했을 떄와 달리 데이터의 추가 삭제가 불가능해진다.
	     단 크기를 변경하지 않는 데이터의 변경은 가능하다. 따라서 고정된 개수의
	     데이터를 저장하거나 활용할떄 사용된다. 
...
*/
	public static void main(String[] args) {
		// Array.asList() 매서드를 사용한 정적 컬렉션 객체 생성 
...
//	   List<Integer> aList6 = new LinkedList<>(30); 저장 용량 지정 불가 
	...
	// 4. addAll - 또 다른 리스트 위치 지정 하여 통쨰로 넣기
	...
	for(int i = 1; i < aList2.size()+1; i++) {
		System.out.println(i + " 번쨰 : " + aList2.get(i-1));
	}
