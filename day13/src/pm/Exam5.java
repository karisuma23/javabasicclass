...
내부 멤버에서 활용하는 클래스를 작성할떄는 제공되는 클래스나 인터페이스의  다양성만큼
많은 가지수의 클래스를 생성해야 한다. 또한 동일한 이름의 매서드가 다양한 타입에 
입력 매개변수를 가질수 있도록 하려면 고려하는 입력매개변수 타입의 수만큼 오버로딩을 수행해야한다. 
그래서 이러한 비효율성을 한번에 해결하는데 필요한 문법 요소가 바로 '제네릭' 이다.

1. 제네릭을 사용하지 않고 여러 객체를 저장하는 클래스를 작성해보자
   ...
   class Goods1{
    ...
    Pencil pencil = goods2.get();
       ...
       먼저 클래스 Goods1 Apple타입의 필드, 이 필드값을 쓸수 있는 getter매서드와
    setter 매서드를 갖고 있다. 따라서 이클래스의 객체를 생성함으로 사과를 저장 setter 하고 
       저장된 사과를 가져오는 get기능을 수행할 수 있다. 
    
       
       클래스 Goods2는 연필을 저장하고 관리할수 있는 클래스로 필드가 pencil 타입
       이라는 것만 제외하면 클래스 Goods1 과 동일하다.
