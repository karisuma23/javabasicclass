...
}                           추상 메서드를 포함하는 클래스는 추상 클래스로 정의해야 한다. 
...
다형적 표현을 사용할떄도 cry()매서드를 호출한다.

추상 매서드 사용의 장점

일반 클래스에 정의 했을떄와 추상클래스로 정의 했을떄
자식 클래스에서 cry() 매서드를 오버라이딩 하는 과정에서 만약에 매서드명에
오타가 발생됐다고 가정했을떄  cry()...
이떄 일반 클래스를 상속한 자식 클래스에서는 오버라이딩이 아니라 추가로 새로운 매서드를 정의한 결과가 된다. 
따라서. cry() cRY() 2개의 매서드가 존재하게 된다. 이런 경우 객체를 생성하고 
cry() 매서드를 호출해도 아무것도 출력되지 않는다. 부모의 cry()매서드에는 아무것도 정의 되지않았기 
때문이다. 
추상 클래스를 생각해보면... 
자식 클래스가 오버라이딩 하려고하는 과정에서 오타가 발생하면 문법오류가 발생한다. 
추상 클래스를 상속 받으면 추상클래스에 정의 해놓은 추상 매서드들을 
...
정리...
만약 abc()라는 추상 매서드를 포함하고 있는 추상 클래스가 있을떄 '이것을 상속한
