...
/*
	...
	2가지 방법 
	1. 쓰레드를 클래스를 상속받아 run()매서드를 오버라이딩 하는 방법 
	2. 1단계 - Runnable 인터페이스를 구현 Runnable 객체를 생성한다.
	   2단계 - 쓰레드 객체를 생성할 떄 1단계에서 생성한 Runnable 객체를 쓰레드 생성자의 매개변수로 전달해준다. 
	   
	사실 2가지 생성방법 모두 run()매서드를 재정의 하여 Thread 객체를 생성한다. 

	쓰레드 실행 방법

	객체를 어떤 방법으로 실행하든 쓰레드 객체 내의 start()매서드를 호출하다.
	여기서 재정의한 매서드는 run()인데 run()의 내용을 실행하기 위해서는 반드시
	start()매서드를 호출해야 한다는 것을 명심하자. 주의할 점은 start()매서드로
	한번 실행된 쓰레드 객체는 재사용 할수 없다는 것이다. 만약 다시 실행하려면 객체를 
	다시 생성해야만 한다. 
...
/*
 ...
 프로그램이 처음 실행될떄 이미 main쓰레드가 실행되고 있으므로 이 2개의 작업을 동시에 실행하기 위해서는
 적어도 하나 이상의  쓰레드를 추가적으로 생성하여 실행해야 한다. 
 ...
 이런 이유로 자막 번호가 항상 비디오 번호 뒤에 나오도록 쓰르드에 Thread.sleep(10)을 추가해서
 0.01초 늦게 출력되도록 했다. 
 */
